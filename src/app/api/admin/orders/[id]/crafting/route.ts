...
type OrderRow = {
  ...
  crafting_email_sent_at: string | null;
};
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await admin
    .from("orders")
    .select("id,order_id,status,shipping_email,shipping_name,items,crafting_email_sent_at")
    .eq("id", params.id)
    .maybeSingle();
  if (error) {
    console.error("Crafting order load failed:", error);
    return NextResponse.json({ error: "Unable to load order" }, { status: 500 });
  }
  if (!data) return NextResponse.json({ error: "Order not found" }, { status: 404 });
  const order = data as OrderRow;

  if (order.status === "printing" && order.crafting_email_sent_at) {
    return NextResponse.json({ order, emailSent: true, alreadyProcessed: true });
  }
  if (["shipped", "delivered", "cancelled"].includes(order.status)) {
    return NextResponse.json(
      { error: `A ${order.status} order cannot move back to crafting` },
      { status: 409 }
    );
  }
  ...
  if (order.status !== "printing") {
    const { data: statusRow, error: updateError } = await admin
      ...
      .single();
    if (updateError) {
      console.error("Crafting update failed:", updateError);
      return NextResponse.json({ error: "Unable to update the order status" }, { status: 500 });
    }
    updated = statusRow;
  }

  // The crafting notification is sent at most once, even if the status
  // later moves back and forth between stages.
  if (order.crafting_email_sent_at) {
    return NextResponse.json({ order: updated, emailSent: true, alreadyProcessed: true });
  }
  ...
  if (!emailResponse.ok) {
    console.error("Crafting email failed:", await emailResponse.text());
    return NextResponse.json(
      {
  ...
  const { data: finalized, error: finalizeError } = await admin
    .from("orders")
    .update({ crafting_email_sent_at: new Date().toISOString() })
    .eq("id", order.id)
    .select("*")
    .single();
  if (finalizeError) console.error("Crafting email marker update failed:", finalizeError);
